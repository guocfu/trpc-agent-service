...
统一装配由 tenant 的 `TenantBackendProfile` 决定：state 为 `redis|sql`，artifact 为 `s3`，knowledge/audit 为 `sql`。接口分别复用 SDK Session/Memory/Knowledge/Artifact 抽象，平台只负责 resolver、SQL 元数据、S3 服务和审计 repository。没有独立向量库或外部 Memory SaaS 实现；SQL Knowledge 是当前可运行的知识检索后端。
